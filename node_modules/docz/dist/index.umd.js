...
var observe = _interopDefault(require('callbag-observe'));
var makeSubject = _interopDefault(require('callbag-subject'));
var reactRouter = require('react-router');
var db = _interopDefault(require('~db'));
...
var tag = require('@mdx-js/tag');
var fp = require('lodash/fp');
var mdxProvider = require('@mdx-js/tag/dist/mdx-provider');
var importedComponent = _interopDefault(require('react-imported-component'));
var React = require('react');

function create(initial = {}) {
    var _a;
    const subject = makeSubject();
    const { Provider, Consumer } = React.createContext(initial);
    Consumer.displayName = 'StateConsumer';
    return {
        get: fn => React.createElement(Consumer, null, fn),
        set: fn => subject(1, fn),
        Provider: (_a = class CustomProvider extends React.Component {
                constructor() {
                    super(...arguments);
...
const ErrorReporter = ({ error, componentStack }) => (React.createElement("div", { style: styles.wrapper },
    React.createElement("div", { style: styles.page },
        React.createElement("h1", { style: styles.title }, "\uD83D\uDEA8 Whoops, we crashed \uD83D\uDEA8"),
        error && React.createElement("div", null, error.message),
        React.createElement("h2", { style: styles.subtitle }, "Stack trace"),
        componentStack && (React.createElement("div", { style: styles.stack }, componentStack.split('\n').map(str => (React.createElement("div", { key: str }, str))))))));
class ErrorBoundary extends React.Component {
    ...
    render() {
        ...
        return error !== null ? React.createElement(ErrorReporter, Object.assign({}, this.state)) : children;
    }
}
...
const ScrollToTop = reactRouter.withRouter(ScrollToTopBase);
...
function theme(themeConfig, transform = c => c) {
    return WrappedComponent => {
        const Theme = props => {
            ...
            const Router = (props) => Boolean(hashRouter) ? (React.createElement(reactRouterDom.HashRouter, Object.assign({}, props))) : (React.createElement(reactRouterDom.BrowserRouter, Object.assign({}, props)));
            const wrapped = Wrapper ? (React.createElement(Wrapper, null,
                React.createElement(WrappedComponent, null))) : (React.createElement(WrappedComponent, null));
            return (React.createElement(ErrorBoundary, null,
                React.createElement(state.Provider, { initial: Object.assign({}, db, { themeConfig, transform }) },
                    React.createElement(DataServer, { websocketUrl: props.websocketUrl },
                        React.createElement(Router, { basename: BASE_URL },
                            React.createElement(ScrollToTop, null, wrapped))))));
        };
        return Theme;
    };
}
...
function mergeArrBy(prop, a, b) {
    return fp.unionBy(prop, a)(b);
}

const Docs = ({ children }) => {
    if (typeof children !== 'function')
        return null;
    return (React.createElement(React.Fragment, null, state.get(({ entries, config }) => {
...
const Link = props => React.createElement(reactRouterHashLink.NavHashLink, Object.assign({}, props, { exact: true }));
...
const normalize = (item) => {
    ...
    return Object.assign({}, selected, { id: selected.id || ulid.ulid(), order: selected.order || 0, menu: Array.isArray(selected.menu)
...
const BasePlayground = ({ components, className, style, wrapper: Wrapper, children, __scope, __position, __code, __codesandbox, }) => {
    if (!components || !components.render)
        return null;
    return (React.createElement(components.render, { className: className, style: style, components: components, component: Wrapper ? React.createElement(Wrapper, null, children) : children, scope: __scope, position: __position, code: __code, codesandbox: __codesandbox }));
};
const Playground = mdxProvider.withMDXComponents(BasePlayground);
...
const getPropType = (prop, Tooltip) => {
    ...
    return prop.flowType ? (React.createElement(Tooltip, { text: humanize(prop.flowType) }, name)) : (React.createElement(Tooltip, { text: humanize(prop.type) }, name));
};
const BasePropsTable = ({ of: component, components }) => {
    ...
    const hasDescription = Object.keys(props).some((name) => {
        const description = fp.get(`${name}.description`, props);
        return Boolean(description) && Boolean(fp.get('length', description));
    });
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(Table, { className: "PropsTable" },
            React.createElement(Thead, { style: styles$1.thead },
                React.createElement(Tr, null,
                    React.createElement(Th, { className: "PropsTable--property" }, "Property"),
                    React.createElement(Th, { className: "PropsTable--type" }, "Type"),
                    React.createElement(Th, { className: "PropsTable--required" }, "Required"),
                    React.createElement(Th, { className: "PropsTable--default" }, "Default"),
                    hasDescription && (React.createElement(Th, { width: "40%", className: "PropsTable--description" }, "Description")))),
            React.createElement(Tbody, null, props &&
                Object.keys(props).map((name) => {
                    ...
                    return (React.createElement(Tr, { key: name },
                        ...
                        React.createElement(Td, null, getPropType(prop, Tooltip)),
                        React.createElement(Td, null, String(prop.required)),
                        !prop.defaultValue ? (React.createElement(Td, null,
                            React.createElement("em", null, "-"))) : (React.createElement(Td, null, prop.defaultValue.value === "''" ? (React.createElement("em", null, "[Empty String]")) : (prop.defaultValue &&
                            prop.defaultValue.value.replace(/\'/g, '')))),
                        hasDescription && (React.createElement(Td, null, prop.description && prop.description))));
                })))));
};
const PropsTable = mdxProvider.withMDXComponents(BasePropsTable);

const ThemeConfig = ({ children }) => {
    if (typeof children !== 'function')
        return null;
    return (React.createElement(React.Fragment, null, state.get(({ transform, config, themeConfig = {} }) => {
...
class AsyncComponent extends React.Component {
    ...
    render() {
        ...
        return React.createElement(Comp, Object.assign({}, props, { data: Object.assign({}, data, { loading, error }) }));
    }
    async fetch() {
...
async function loadFromImports(path) {
    ...
    const { imports } = await Promise.resolve(require('~imports'));
    const { default: Component, getInitialData } = await imports[path]();
    const ExportedComponent = props => (React.createElement(AsyncComponent, Object.assign({}, props, { as: Component || 'div', getInitialData: getInitialData })));
    return mdxProvider.withMDXComponents(ExportedComponent);
}
...
const AsyncRoute = (_a) => {
    ...
    return Page ? (React.createElement(Page, Object.assign({}, props),
        React.createElement(Component, Object.assign({}, props)))) : (React.createElement(Component, Object.assign({}, props)));
};

const DefaultLoading = () => React.createElement(React.Fragment, null, "Loading");
const Identity = ({ children }) => (React.createElement(React.Fragment, null, children));
const DefaultRender = ({ component, code }) => (React.createElement(React.Fragment, null,
    component,
    code));
...
const DocPreview = ({ components: themeComponents = {}, }) => {
    ...
    return (React.createElement(tag.MDXProvider, { components: components }, state.get(({ entries }) => {
        if (!entries)
            return null;
        return (React.createElement(reactRouterDom.Switch, null,
            Object.keys(entries).map(path => {
                const entry = fp.get(path, entries);
                const props = { path, entries, components };
                const component = loadRoute(path, components.loading);
                component.preload();
                return (React.createElement(reactRouterDom.Route, { exact: true, key: entry.id, path: entry.route, render: routeProps => (React.createElement(AsyncRoute, Object.assign({}, routeProps, props, { asyncComponent: component }))) }));
            }),
            NotFound && React.createElement(reactRouterDom.Route, { component: NotFound })));
    })));
};
