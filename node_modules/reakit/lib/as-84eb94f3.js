...
function parseTag(tag) {
  if (Array.isArray(tag)) {
    var tags = tag.filter(function (currentTag, i) {
      return typeof currentTag !== "string" || i === tag.length - 1;
    });
...
function As(props) {
  return render(Object.assign({}, omit(props, "nextAs"), {
    as: props.nextAs
  }));
}

function render(props) {
  var T = parseTag(props.as);

  if (Array.isArray(T)) {
    var _T$filter = T.filter(function (x) {
      return x !== As;
    }),
        First = _T$filter[0],
        others = _T$filter.slice(1);

    var other = others.length === 1 ? others[0] : others; // @ts-ignore: We can't be sure if First accepts `as` or `nextAs`

    return React.createElement(First, __chunk_1._extends({}, props, {
      as: As,
      nextAs: other
    }));
  }
  ...
  if (typeof T === "string") {
    ...
    var _allProps = Object.assign(pickHTMLProps(props), {
      className: className
    }, style ? {
      style: style
    } : {});

    return React.createElement(T, __chunk_1._extends({}, _allProps, {
      ref: props.elementRef
    }));
  }

  var allProps = Object.assign(omit(props, "as"), style ? {
    style: style
  } : {});
  return React.createElement(T, allProps);
}
...
function as(asComponents) {
  return function (WrappedComponent) {
    ...
    var defineProperties = function defineProperties(scope) {
      scope.asComponents = asComponents; // @ts-ignore

      scope.as = function (otherComponents) {
        return as(otherComponents)(scope);
      };
    ...
    if (isAsComponent(WrappedComponent) && asComponents === WrappedComponent.asComponents) {
      return defineProperties(WrappedComponent);
    }

    var componentName = getComponentName(WrappedComponent);
    var commaSeparatedAs = [].concat(asComponents).map(getComponentName);
    var displayName = componentName + ".as(" + commaSeparatedAs + ")";

    var EnhancedComponent = function EnhancedComponent(props) {
      return render(Object.assign({}, props, {
        as: [WrappedComponent].concat(toArray(asComponents), toArray(props.as || []), toArray(props.nextAs || []))
      }));
    };

    EnhancedComponent.displayName = displayName; // @ts-ignore: Only docs

    EnhancedComponent.propTypes = WrappedComponent.propTypes; // @ts-ignore: Only docs

    EnhancedComponent.defaultProps = WrappedComponent.defaultProps; // @ts-ignore
